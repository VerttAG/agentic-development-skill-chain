...
>
> **Before executing:** ensure `scripts/wave-gate.sh` exists in the project root. If missing, copy from `../5-executing/scripts/wave-gate.sh` and commit (`chmod +x` required). Also install `jq`, `coderabbit`, `agent-browser` if missing — the script needs them.
>
> Ready to execute! Use the **executing skill** (`/5-executing`) to implement wave by wave.
