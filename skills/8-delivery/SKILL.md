---
name: 8-delivery
description: "Run P8 delivery: probe merge conflicts against main, render the PR body from state.json and findings.json, create the PR via gh, poll CI with a bounded fix loop, and reconcile Checkpoint 2 review comments."
license: MIT
compatibility: Requires git, jq, node, and the gh CLI authenticated against the repository remote.
---
...
Every gate, template and artifact here is host-neutral. The only difference is how a fix task is dispatched:

- **Delegation available:** dispatch each fix as a worker named `micro-fixer` (Claude Code: the `Agent` tool; Codex: `spawn_agent` with the `worker` or default role, where the session policy allows delegation). Do not assume `subagent_type` or `run_in_background` exist on hosts other than Claude Code.
- **No delegation:** apply the same fix inline, one cluster at a time, with the failing output or review comment quoted verbatim. The CI loop and the CP2 reconcile are unchanged.
...
- Exit 1 (red) → the script printed the failing checks and verbatim
  `--log-failed` output. Dispatch a fix task named `micro-fixer` with EXACTLY that
...
2. `fix now` → a fix task named `micro-fixer` (comment verbatim + file paths),
   verify, commit.
