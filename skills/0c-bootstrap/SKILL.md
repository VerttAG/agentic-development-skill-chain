---
name: 0c-bootstrap
description: "Turn an approved product vision into a running empty project: decide the stack into docs/ARCHITECTURE.md, run the real scaffold, verify build and tests green, write AGENTS.md. Use when there is no application code yet."
license: MIT
---
...
with code is `0b-intake`'s job — extraction, not creation. Check first.
</HARD-GATE>
...
| Components | shadcn/ui | | themed by `1c-frontend-design` |
...
<!-- Everything below is filled by 3-architecture (per PROJ) and P7 curation. -->
```
...
`0b-intake`), do not rewrite it — insert or correct the `## Stack` section
...
**Root `CLAUDE.md`** — write it only when the project is also used from
Claude Code. When present it is a pointer, never a second rulebook:
...
Most other hosts (Codex, Cursor, Amp, OpenCode, Gemini CLI …) read
`AGENTS.md` natively, so they need no extra file at all. Two rule files that
