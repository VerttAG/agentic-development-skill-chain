---
name: 4a-checkpoint
description: "Run a human checkpoint as a reconcile loop: compact review package, feedback point by point (adopt/change/reject/defer), decision log, cascade through affected artifacts, seal state.json. Use for Checkpoint 1 or Checkpoint 2."
license: MIT
---
...
| **Bootstrap** (via 0b-intake) | intake first drafts (all eight baseline files) | curated baseline commit — no state.json |
...
Walk the package one point at a time — never one bulk "any comments?"
prompt. Use a structured-choice tool if the host has one (Claude Code:
`AskUserQuestion`); otherwise walk the points as guided conversation, one
at a time. For each point:
...
1. Read the `state.json` created with the PROJ folder by brainstorming (1).
   ...
   `bash ../4b-setup/scripts/state.sh init <X> <theme>`;
...
## Bootstrap Variant (via 0b-intake)
