---
name: 0b-intake
description: "Bootstrap the curated docs baseline for a repo that has code but no docs/PRODUCT.md: provenance-marked drafts from a code scan, developer interview, checkpoint reconcile, sealed commit. Also for an explicit docs drift audit."
license: MIT
compatibility: Requires git and bash; node only when context bundles are compiled.
---
...
detected inconsistency. Work through it point by point — grouped by
topic, never as one wall of questions. Use a structured-choice tool if the
host has one (Claude Code: `AskUserQuestion`); otherwise ask one topic per
turn as a numbered block and wait for the answers before the next block.
...
2. `bash scripts/intake-seal-check.sh` (copy from
   `../0b-intake/scripts/`, and `curation-caps.sh` from
   `../7-documentation/scripts/`, if missing). It verifies: all eight files exist,
