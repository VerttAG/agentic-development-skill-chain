#!/usr/bin/env bash
# intake-seal-check.sh — seal gate for the 0b-intake bootstrap (CONCEPT.md §3).
#
...
# curation-caps.sh is owned by the 7-documentation skill. Prefer the project
# copy (P0 installs one), then a sibling next to this script, then the skill.
resolve() { if [ -f "$1" ]; then echo "$1"; else echo "$SCRIPT_DIR/$2"; fi; }
CAPS="$(resolve scripts/curation-caps.sh ../../7-documentation/scripts/curation-caps.sh)"
[ -f "$CAPS" ] || { echo "MISSING curation-caps.sh (looked in scripts/ and ../../7-documentation/scripts/)"; exit 1; }
if ! bash "$CAPS" --require-baseline .; then
  FAIL=1
fi
