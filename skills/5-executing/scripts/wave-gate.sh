...
ADVISORY_CONFIGURED=()
while IFS= read -r _sev; do ADVISORY_CONFIGURED+=("$_sev"); done \
  < <(jq -r "${WAVE_KEY}.advisory_severities[] | ascii_downcase" "$CFG")
...
if [[ -n "$WAVE_BASE" ]]; then
  ...
  [[ -n "$WAVE_BASE" ]] || fail "no wave base SHA — set env WAVE_BASE_SHA or create tag '${WAVE_TAG}' before running the gate. Skill 5 Step 2a sets this tag for Wave 1; later waves are tagged automatically by the previous gate."
...
ROUTES=()
while IFS= read -r _route; do ROUTES+=("$_route"); done \
  < <(jq -r "${WAVE_KEY}.frontend_routes[]? // empty" "$CFG")
if [[ "${#ROUTES[@]}" -eq 0 ]]; then
...
if [[ -f "$NEXT_PLAN" ]]; then
  ...
  echo "  After Quality Gate passes → /compact → invoke /6-qa (see Skill 5 Step 11 handoff)."
fi
