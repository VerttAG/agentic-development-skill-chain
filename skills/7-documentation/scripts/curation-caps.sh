...
#   AGENTS.md (root)       <= 40 non-blank lines   (existing 7-documentation cap)
#
...
#                       (used by 0b-intake's seal check; normal P7 runs on
...
missing() { # <file>
  if [ "$REQUIRE_BASELINE" -eq 1 ]; then
    ...
    echo "WARN    $1 missing — no baseline yet (not a breach; run 0a/0c for a new build, 0b-intake for an existing codebase)"
  fi
}
