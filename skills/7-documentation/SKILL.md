...
description: "Curate human documentation after QA passes: README.md, docs/PROJECT.md, docs/TECHNICAL.md, approved AGENTS.md entries, and the long-lived docs/ baseline behind the P7 form and truth gates. Not for PRDs or API reference."
license: MIT
---
...
Gates, output structure, diagrams, content ownership, and `AGENTS.md` merge behavior are the same on every host. Only tool invocation differs:

- Durable agent context always lives in project-root `AGENTS.md`.
- Write project-root `CLAUDE.md` only when the project is also used from Claude Code. When present it must stay pointer-only: it references `AGENTS.md` as required reading and carries no curated rules.
- Always ask the user before merging an `AGENTS.md` candidate, unless the current request explicitly authorizes automatic merging.
- **Delegation to subagents:** use it when the host offers it and it is useful (Claude Code: the `Agent` tool; Codex: `spawn_agent` roles where the session's policy allows). Otherwise generate the docs locally.
...
Use the template in [`assets/project-md-template.md`](assets/project-md-template.md).
...
Use the template in [`assets/readme-template.md`](assets/readme-template.md).
...
Use the template in [`assets/technical-md-template.md`](assets/technical-md-template.md).
