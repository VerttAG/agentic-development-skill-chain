---
name: 6-qa
description: "Test a finished PROJ end to end against acceptance criteria: adversarial and edge-case testing, security audit, UI consistency and a simplicity review. Finds and documents bugs, never fixes them. Use before release."
license: MIT
compatibility: Requires node for the findings ledger. Browser automation needed for the end-to-end pass.
---
...
The gates, persona roles, severity rules, durable-context behavior, and handoff are the same on every host. Only tool invocation differs — follow the capability you actually have:

- **Browser automation** (Playwright, agent-browser, or the host's own): use it for the E2E pass. Without it, run the repo's test commands and report the E2E gap explicitly — never claim coverage you did not have.
- **Delegation to subagents:** run the six-persona panel in parallel, one worker per persona (Claude Code: the `Agent` tool with `subagent_type: general-purpose`; Codex: `spawn_agent` roles, and only where the session's policy allows delegation). Without delegation, run the six reviews sequentially in the main session against focused diffs and keep each report short.
- **Context compaction:** hosts that compact automatically need no action; on hosts with an explicit command (Claude Code: `/compact`) run it where this skill says to.
- Durable context always goes to project-root `AGENTS.md`. Keep `CLAUDE.md` pointer-only, and only when Claude compatibility is expected.
- Keep long screenshots, transcripts, logs, and persona reviews out of the final answer; summarize with file/line, reproduction, severity, and evidence path.
...
- If production-ready (no Critical/High): invoke `/7-documentation` for PROJ-<X>
