...
description: "Step 1 of the PROJ workflow, before creating or changing any feature, component, workflow or behavior. Turns an idea into approved buildable concepts via discovery, scope decomposition, exploration and risk review."
license: MIT
---
...
On the discovery track against an existing product, capture the existing state BEFORE exploring approaches — surfaces, design system, domain vocabulary, constraints and open questions — so the concept builds on what is really there.

The procedure and the `existing-state.md` template: [`references/brownfield-intake.md`](references/brownfield-intake.md).

## Project Decomposition Gate

Before writing a concept, decide whether the seed is ONE PROJ or several. A seed that spans multiple independent products, surfaces or delivery tracks must be decomposed, and the split needs explicit user approval before any concept is written.

When to decompose, the decomposition output format, the user-approval rules and how decomposition is recorded in concept documents: [`references/decomposition-gate.md`](references/decomposition-gate.md).
...
14. **Create PROJ folder and state** - create `specs/PROJ-<X>-<theme>/1_brainstorm/`, then run `bash ../4b-setup/scripts/state.sh init <X> <theme>`. The new file stays `CP1:pending`; only checkpoint (4a) may approve it.
