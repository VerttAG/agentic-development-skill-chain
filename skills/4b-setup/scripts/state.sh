...
#   2  state.json missing (run 'state.sh init' — owned by 4a-checkpoint at CP1)
#   64 usage error
set -euo pipefail
...
require_state() { [ -f "$STATE" ] || { echo "state.sh: $STATE missing (create it via 'state.sh init' — owned by 4a-checkpoint)" >&2; exit 2; }; }
