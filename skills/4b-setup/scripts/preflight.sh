...
resolve_helper() { # <name> -> path on stdout, or return 1
  ...
  for p in "$SCRIPT_DIR/$n" \
           ${SKILL_CHAIN_HOME:+"$SKILL_CHAIN_HOME/4b-setup/scripts/$n"} \
           "$HOME/.agents/skills/4b-setup/scripts/$n" \
           "$HOME/.claude/skills/4b-setup/scripts/$n" \
           "$HOME/.codex/skills/4b-setup/scripts/$n"; do
    [ -f "$p" ] && { echo "$p"; return 0; }
  done
  return 1
}
...
if [ -f "$BASE/context/bundles.lock.json" ]; then
  if COMPILER="$(resolve_helper compile-context-bundles.mjs)" \
    ...
    say "❌ context bundles verify FAILED (budget breach, drift, or compiler missing — hard) — condense docs/, recompile (4b-setup: Compile the context bundles)"
    HARD_MISSING+=("context-bundles")
  fi
else
  say "– context bundles not compiled yet (4b-setup compiles them after this preflight)"
fi
...
# 1b-visual-companion (recorded under "Design System State" in
...
if [ "$HAS_COMPONENTS" = true ]; then
  if [ ! -f docs/components.md ]; then
    say "– docs/components.md missing but components exist — generate it: node scripts/gen-component-registry.mjs (installed by 4b-setup: Copy framework scripts)"
...
if [ -f docs/DESIGN-SYSTEM.md ]; then
  ...
  say "– no docs/DESIGN-SYSTEM.md — UI work without design rules; run 1c-frontend-design, or proceed knowingly (a knowing skip recorded in layout-decision.md is expected here — do not re-ask)"
...
if [ -f "$BASE/state.json" ]; then
  ...
  say "⚠ $BASE/state.json missing — report not persisted (run 4a-checkpoint first)"
fi
