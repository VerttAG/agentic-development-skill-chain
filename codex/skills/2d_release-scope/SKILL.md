---
name: release-scope
description: "Classify a phase-tagged feature inventory into an executable release slice that cuts across many PROJs. Resolves every in-phase feature ID to the PRD user stories that deliver it, lists the later-phase stories that must be excluded from in-scope PRDs, finds the features with no PRD behind them, and consolidates the forward-compatibility obligations that let later phases stay high-level without becoming blockers. Use after requirements-engineer (2) when a roadmap splits work into phases/waves that each span several PROJs, and before architecture (3) so the tech design is scoped to the release rather than to one PROJ. Not for: per-PROJ scoping (the PRD manifest covers that), writing the missing PRDs it finds, or resolving PRD review feedback (use review-reconcile, 2c)."
---
